...
- Extension installation failures now remain visible so users can retry or dismiss them.
- Settings no longer crash when the settings layout changes from one pane to two panes.
- Player settings no longer crash when the settings layout changes from one pane to two panes.
...
### Fixed
- Reader EPUB loading now skips image references to files that the archive does not contain.
...
- An anime download directory that cannot be created now fails the one episode instead of stopping the whole download queue.
- Anime downloads to a storage location without a local file path no longer crash the external-downloader hand-off.
- A source or anime folder deleted while the download cache scans it no longer leaves the download index stuck.
...
- The previous and the next chapter now show translated pages immediately after a translate toggle. Before, they kept the pages of the earlier setting until the chapter was reopened.
...
- Failed chapters now show an explicit "Failed" label in the translation summary card instead of a bare chapter name.
...
- Translated pages are now stored next to chapters that are downloaded as CBZ archives, and the reader shows them when translated pages are on.
...
- The player can load its media libraries again on a 64-bit ARM device. The app packaged a version of FFmpeg that the player library cannot use.
- The player no longer crashes when you start an episode. The app spoke to the media engine before the engine had started.
- The audio channel setting now applies to playback. The app sent it to the media engine before the engine had started, and the setting was lost.
...
### Changed
- Extension names now strip both `Rayniyomi: ` and `Aniyomi: ` prefixes for smooth migration from Aniyomi extension repos
- All tracker and service HTTP clients now identify as `Rayniyomi` in the User-Agent header
...
- **Custom accent a11y guardrails** — custom accent settings now include contrast-risk warnings across light/dark/AMOLED modes, live-region accessibility announcements for swatch/apply actions, and deterministic semantics traversal tags for picker controls
