...
### Fixed
- Manga extensions that declare TachiyomiX lib 1.6 can now browse and read. The extensions installed and registered their sources, but every network call failed with `IgnoreGzipInterceptor must not be present in default client`, because the TachiyomiX 1.6 library refuses a default client that strips gzip. The app no longer adds the gzip and Brotli network interceptors, so OkHttp handles gzip transparently. Lib 1.4 extensions are unaffected.
