...
fun MangaTrackInfoDialogHome(
    trackItems: List<MangaTrackInfoItem>,
    dateFormat: DateTimeFormatter,
    onStatusClick: (MangaTrackInfoItem.Tracked) -> Unit,
    onChapterClick: (MangaTrackInfoItem.Tracked) -> Unit,
    onScoreClick: (MangaTrackInfoItem.Tracked) -> Unit,
    onStartDateEdit: (MangaTrackInfoItem.Tracked) -> Unit,
    onEndDateEdit: (MangaTrackInfoItem.Tracked) -> Unit,
    onNewSearch: (MangaTrackInfoItem) -> Unit,
    onOpenInBrowser: (MangaTrackInfoItem.Tracked) -> Unit,
    onRemoved: (MangaTrackInfoItem.Tracked) -> Unit,
    onCopyLink: (MangaTrackInfoItem.Tracked) -> Unit,
    onTogglePrivate: (MangaTrackInfoItem.Tracked) -> Unit,
) {
    Column(
        modifier = Modifier
            .animateContentSize()
        ...
        trackItems.forEach { item ->
            when (item) {
                ...
                is MangaTrackInfoItem.Untracked -> {
                    TrackInfoItemEmpty(
                        tracker = item.tracker,
                        onNewSearch = { onNewSearch(item) },
                    )
