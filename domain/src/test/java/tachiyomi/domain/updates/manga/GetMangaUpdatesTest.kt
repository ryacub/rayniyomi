...
class GetMangaUpdatesTest {
    ...
    fun `old subscribe overload still delegates unchanged`() {
        ...
        every { repository.subscribeAllMangaUpdates(after = 2_345_678L, limit = 500L) } returns emptyFlow()

        interactor.subscribe(instant)

        verify(exactly = 1) {
            repository.subscribeAllMangaUpdates(after = 2_345_678L, limit = 500L)
        }
    }
}
